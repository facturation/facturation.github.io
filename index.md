...
<%= api_block(true) do %>
<%= curl_cmd(user_agent_key: 'X-User-Agent') %> "<%= firm_customers_url(firm_id: api_firm_id, format: :json) %>"
<% end %>
...
* login API : <%= current_user.id %>
* mot de passe API : <%= current_user.api_key %>

A tout moment, vous pouvez <%= link_to("générer un nouveau mot de passe API", user_api_url, method: :put, data: {confirm: "Etes-vous sur de vouloir re-générer votre clé API?"}) %>
  <% else %>
  <p><strong>Pour obtenir vos codes d'accès à l'API, vous devez au préalable activer votre compte à l'aide du lien que nous vous avons transmis par courriel.</strong></p>
  <% end %>
<% else %>
<strong>Vous devez vous connecter au service pour obtenir vos codes d'accès à l'API.</strong>
<% end %>
...
# Paramètre FIRM_ID
  ...
  <%= render partial: 'firm_ids' %>
<% end %>

<%= render partial: 'testing' %>

<%= definition_block("Lecture des données") do %>
...
<%= definition_block("Ecriture des données") do %>
...
<%= api_content('"Content-type: application/json; charset=utf-8"') %>. Il vous suffit alors d'inclure vos données au format JSON dans le corps de votre requête pour que l'API effectue l'opération demandée.
...
<%= definition_block("Champs disponibles") do %>
A la fin de chaque page de documentation de l'API, nous vous indiquons la liste des champs disponibles pour l'objet concerné, avec pour chaque champs le type de donnée qu'il accepte et si ce champs est accessible en écriture ou non.<br/>
...
<%= definition_block("Pagination") do %>
...
<%= definition_block("Gestion des erreurs") do %>
...
<%= definition_block("Limitations du nombre de requêtes") do %>
...
<%= definition_block("Compatibilité JSONP") do %>
